...
void printASTNode(ASTNode *node) {
    ...
    switch (node->type) {
        case NODE_VAR_DECL:
            ...
            printf("Variable Declaration: %s %s\n", tokenTypeToString(node->data.var_decl.var_type), node->data.var_decl.var_name);
        ...
        case NODE_EXPRESSION:
            ...
            // Assurez-vous d'afficher correctement les informations sur le nœud d'expression
            if (node->data.var_decl.initial_value) {
